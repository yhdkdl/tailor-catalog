...
export function MarketplaceViewClient({ designs, categories, initialTrending = false }: { designs: MarketplaceDesign[]; categories: CatalogCategory[]; initialTrending?: boolean }) {
  ...
  return <main className="min-h-screen bg-surface-950 text-slate-100 px-4 py-6 sm:px-8">
    <div className="mx-auto max-w-7xl space-y-5">
      ...
      {filtered.length === 0 ? <p className="py-16 text-center text-slate-400">{t('catalog.no_designs')}</p> : <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{filtered.map((d) => <DesignCard key={d.id} design={d} shopName={d.tailor.shop_name} shopSlug={d.tailor.shop_slug} onInspect={(design) => setSelected(design as MarketplaceDesign)} onView360={(design) => setViewer(design as MarketplaceDesign)} />)}</div>}
